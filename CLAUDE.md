...
3. 反転対象の line を **8 方向（u/d/l/r/ul/dr/ur/dl）それぞれを 1 グループ** に仕分け、**グループを枚数降順（同数時は u>d>l>r>ul>dr>ur>dl）で順次実行**。これで 1 手の中で複数方向が同時にフリップすることはなくなる（issue #1 の修正）。グループ内は `Promise.all` で line を並列起動（実運用では 1 方向に line は高々 1 本なので実質シーケンシャル）、line 内は 200ms 間隔。各石は `flipPieceAnimated()` を起点に「フリップアニメ開始」と同時に `state.board[fr][fc]` を更新。
